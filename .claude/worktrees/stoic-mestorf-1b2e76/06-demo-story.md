# Demo Story: Pitches & Walkthrough Guide (Judge-Aware)

This guide gives you three versions of your pitch—30 seconds, 60 seconds, and a full 5–7 minute walkthrough—shaped for this specific judge panel.

Practice until it feels natural. Don't memorize word-for-word; use these as guides.

**Key principle:** Frame Afterglow as a *browsing and organization tool* that respects DJ artistry, not as a recommendation engine.

---

## The 30-Second Pitch (Attention & Hook)

**Use this when:**
- A judge asks "What are you building?"
- You have limited time before diving into the demo

**Your goal:** Describe the problem vividly. Hint at a different approach that respects DJ thinking.

---

### 30-Second Script

> "I'm a DJ, and I get asked to play spontaneous gigs—house parties, festivals, pool parties—where I don't know the setup in advance. My library is organized by genre and BPM, but when I'm actually DJing, I navigate by *context and moment*. So great tracks get lost because they don't fit neatly in my folders.
>
> **Afterglow flips that.** Instead of organizing by metadata, we organize by *when you play and why you play it*. You start with a timeline of the day, then browse spatial maps of the performance contexts, then discover your tracks.
>
> The result: you feel more confident prepping for uncertain gigs, and you rediscover material you'd forgotten about—tracks you own that actually fit this moment."

**Timing:** ~30 seconds at natural conversational pace.

**Tone:** Grounded, specific, personal. You're speaking from experience.

**Judge resonance:**
- **Richie Hawtin:** "You're navigating by context"—respects DJ thinking
- **Romain Pouillon:** Specific problem (spontaneous gigs) + practical solution
- **Carlos Caires:** "Context and moment"—musical sophistication
- **JB Thiebaut:** Clear, specific problem statement

---

## The 60-Second Pitch (Full Context + Invitation)

**Use this when:**
- You have a minute to pitch before the walkthrough
- A judge wants more detail before committing to watch the demo

**Your goal:** Tell the full story. Problem, why it matters, your insight, your approach.

---

### 60-Second Script

> "Most DJ tools organize music by technical metadata—genre, BPM, key. That works great if you're preparing for a specific gig where you know exactly what time and context you're playing.
>
> But in real life, DJs get booked spontaneously for events where the setup is uncertain. A house party could start at 6 pm or 10 pm. A festival set could be brunch, afternoon, or late night. You don't know until you arrive.
>
> When you're actually DJing, you don't navigate by BPM. You navigate by *moment and context*: What time of night is it? Is the crowd locked-in and focused, or wandering and social? Am I opening, building momentum, resetting energy, or closing the night?
>
> **Afterglow** lets you organize your library the way you actually think about it. You start with a timeline of the day. Then you explore spatial maps of performance contexts—like a tangible map of your own library.
>
> The result: you can prepare for uncertain gigs in minutes instead of hours. You rediscover tracks you'd forgotten. And you feel *confident* that you have material for whatever moment comes."

**Timing:** ~60 seconds at natural pace.

**Tone:** Narrative, grounded, building to a revelation.

**Structure:**
- Problem statement (metadata mismatch)
- Why it matters (real DJ scenarios)
- Your insight (how DJs actually think)
- Your solution (Afterglow's approach)
- The benefit (speed, rediscovery, confidence)

**Judge resonance:**
- **Richie Hawtin:** Respects DJ thinking over automation
- **Carlos Caires:** "Moment and context"—musical intelligence
- **Romain Pouillon:** Real problem, practical solution
- **Michele Darling:** Confident, intuitive experience
- **JB Thiebaut:** Clear scope and narrative arc

---

## The 5–7 Minute Walkthrough (Full Demo)

**Use this for:**
- The main hackathon pitch (judges have 5–10 minutes per project)
- Close-up demos at your booth

**Your goal:** Walk judges through the three-layer model, tell the Pool Party story, land the rediscovery moment. Emphasize *you* making decisions, *your* library, *your* artistry.

**Timing breakdown:**
- Opening hook (30 sec)
- Setup scenario (30 sec)
- Timeline explanation (20 sec)
- Constellation walkthrough (2.5–3 min)
- Rediscovery moment (1–1.5 min)
- Wrap-up (30 sec)

---

### Full Walkthrough Script with Stage Directions

#### Opening (30 seconds)

**[Stand ready, laptop / screen visible. Friendly, conversational. Make eye contact.]**

> "Okay, so picture this. Your friend texts you: 'Can you DJ our pool party at 4 pm? We'll have maybe 20 people.' You have a couple hours to prep.
>
> Most people open Spotify or start scrolling through folders and feel overwhelmed. You don't know what you have. You dig through your library. The energy doesn't quite match. You're stressed.
>
> **Afterglow** is designed for exactly this moment. Let me show you how it helps you organize and discover your own material."

**[Brief, engaging, sets up the relatable scenario.]**

**Judge resonance:**
- **Richie:** Personal, authentic
- **Romain:** Real workflow problem
- **Michele:** Human-centered problem statement

---

#### Setup the Scenario (30 seconds)

**[Point to the screen. Open Afterglow. Timeline view should be visible.]**

> "You open Afterglow. The first thing you see is a timeline of the day—different contexts where you might play.
>
> Warm-up, Pool Party, Festival Brunch, Post-game, Sunrise Reset.
>
> For your gig, you say: 'I'm playing 4 pm. Pool Party.' You click on it."

**[Click on Pool Party. Constellation view loads. Pause for 2 seconds so judges can take in what they're seeing.]**

**What judges should see:**
- Three visible zones with labels (Arrivals, Lock-in, Wanderers)
- Tracks clustered spatially
- Underplayed signal visible on some tracks

---

#### Main Walkthrough: The Constellation (2.5–3 minutes)

**[Stand back slightly. Give judges time to look at the spatial arrangement.]**

> "Now you see something different. Not a list. A map—a spatial arrangement of your **own library** organized around how you actually use it.
>
> You can see **three zones here**:
>
> On the left, you have the **Arrivals energy**—people are still coming in, settling. You want tracks that welcome people without demanding full attention yet.
>
> In the middle, **Lock-in energy**—people are focused, dancing, fully engaged. You have room to take risks here; the crowd is with you.
>
> And on the right, **Wanderers**—people are chatting, moving around, having conversations. You need music that works *in the background of their night*, not against it.
>
> This isn't someone's algorithm telling you what to play. This is **your library**, organized by the decision-making you already do when you perform."

**[Let them look for 5–10 seconds. Gesture through the zones to show spatial relationships.]**

> "Notice: tracks that work in multiple moments—they're positioned between the zones. A track tagged as *both* Lock-in and Wanderers sits right between them. Spatial proximity shows the relationship."

**[Optional: Hover over a track cluster to show nearby tracks fade in/highlight.]**

> "When you hover near a cluster, related tracks become more visible. It's like moving through the space. The interface responds to where you're exploring."

**[Pause for 3 seconds. Let them absorb the spatial logic.]**

**Judge resonance:**
- **Yan:** Intentional spatial design, responsive interaction
- **Carlos:** Musical decision-making shown spatially
- **Michele:** Intuitive navigation; learnable structure
- **Romain:** Practical browsing method

---

#### The Rediscovery Moment (1–1.5 minutes)

**[Move to the Lock-in zone. Point to or click on an underplayed track. Make eye contact.]**

> "Now, here's where something real happens.
>
> See this track? **'Solar Pool.'** Play count: 2. You'd basically forgotten about it.
>
> But it's right here in the Lock-in zone—exactly where it belongs for a 4 pm pool party where people are actually engaged with the music.
>
> You hover over it, and you remember: 'Oh right. I tagged this years ago. It's **exactly what I need** for this moment. Why did I forget about this?'
>
> That's not recommendation. That's **rediscovery**. That's the moment where you realize: *I have material I'd forgotten. I'm more prepared than I thought.*
>
> And it happened because you were browsing your own library—exploring what you actually have—not scrolling through a list."

**[Click on the track to show detail view.]**

**[Show the track details: artist, duration, tags, play count.]**

> "Look at the tags: Pool Party. Bridge. Lock-in.
>
> This isn't a system deciding anything. *You* tagged this honestly when you added it to your library. The system is just showing you *why it belongs here*. Where your own thinking led you."

**[Pause for 3–5 seconds. Let them absorb the moment.]**

**Judge resonance:**
- **All judges:** Universal moment
- **Richie:** Your decision-making, not automation
- **Carlos:** Musical logic shown clearly
- **Romain:** Real material rediscovered

---

#### Show the Arc (45 seconds)

**[Move back from the Lock-in zone. Gesture to all three zones together.]**

> "So in the few minutes we've been here, what actually happened?
>
> You opened the app. You chose your scenario (Pool Party).
>
> You scanned your library spatially—taking in Arrivals, Lock-in, Wanderers all at once.
>
> You *rediscovered* an underplayed track that fits perfectly.
>
> And now you have a sense of the arc: people arriving → locked in focus → moving and socializing.
>
> You didn't scroll through 50 track names. You *explored your own library like a map*.
>
> And you feel confident. You know you have material for this gig."

---

#### Close (30 seconds)

**[Step back. Make eye contact. Warm, confident tone.]**

> "This concept works for any scenario: house parties, festivals, Burning Man camps, ad hoc gigs. Anywhere you don't know the setup in advance.
>
> The core insight: organize your library the way you actually *navigate it*—by context, moment, and role. Not by metadata alone.
>
> The result: faster prep, real rediscovery, and a browsing experience that feels like exploring *your own knowledge*, not fighting a system.
>
> That's Afterglow."

**[Pause. Open to questions with genuine warmth.]**

> "Any questions?"

---

### Walkthrough Stage Directions Summary

| Moment | What to Show | Action | Judge Focus |
|--------|--------------|--------|-------------|
| **0–30 sec** | (Nothing yet) | Deliver opening. Build empathy for the problem | Richie, Michele |
| **30–60 sec** | Timeline view | Point to phases. Click Pool Party. Pause 2 sec | All |
| **60–80 sec** | (Transition) | Explain the three zones briefly | Carlos, Michele |
| **80–240 sec** | Constellation view | Describe zones. Let them look. Hover to show proximity | Yan, Romain, Carlos |
| **240–360 sec** | Underplayed track | Point out rediscovery moment. Show why it belongs there | All |
| **360–420 sec** | (Full view) | Gesture to the arc. Show confidence it creates | Richie, Michele |
| **420+ sec** | (Ready for Q&A) | Close with summary. Emphasize it's their library, their decisions | All |

---

## Common Judge Questions (And How to Answer)

**After your walkthrough, judges might ask these. Have answers ready.**

### Q: "How is this different from Spotify playlists or a good folder structure?"

**Your answer:**
> "Spotify playlists are external and generic. A folder structure is linear—you have to open each folder and read names.
>
> Afterglow organizes *your library* around *your workflow*. You can see everything at once—your whole Pool Party section—and spatial proximity shows you relationships at a glance.
>
> A good folder structure is a good start. Afterglow is what you do *beyond* that: organize around how you actually navigate during performance prep."

---

### Q: "Does this integrate with djay?"

**Your answer:**
> "We designed it to sit *alongside* djay—you organize in Afterglow, then you play in djay (or your preferred player).
>
> Real SDK integration is a post-hackathon goal. For now, the concept is to prove that this *way of thinking about* library organization actually solves the real problem."

---

### Q: "What if someone has 5,000 tracks?"

**Your answer:**
> "The three-layer model scales. With smart filtering and analytic helpers, you can navigate thousands of tracks.
>
> But for the MVP, we're proving the core concept works with a focused demo library. The question isn't 'does it scale?' but 'does this way of organizing actually help DJs?' If yes, then scaling becomes an engineering problem, not a concept problem."

---

### Q: "Who would actually use this?"

**Your answer:**
> "Emerging and casual DJs—people who get booked spontaneously and feel overwhelmed organizing their library.
>
> But also experienced DJs who want to rediscover underplayed material or explore cross-genre combinations in a more intuitive way.
>
> Anyone who DJs in uncertain scenarios or wants to organize around performance contexts, not just metadata."

---

### Q: "How do tracks get tagged?"

**Your answer:**
> "Right now, manual tagging. You listen to a track and ask three questions:
>
> 'When would I play this?' (Time/Setting)
> 'What does it do in a set?' (Journey Role)
> 'What crowd can it handle?' (Crowd State)
>
> The tagging system reflects how DJs already think. It's not reductive; it's precise.
>
> Post-hackathon, we could suggest tags based on metadata (BPM, genre). But the strength is in honest, intentional tagging."

---

### Q: "Doesn't this risk turning DJing into paint-by-numbers?" (Richie might ask)

**Your answer:**
> "No. This is *organization*, not curation. Your taste and choices remain central.
>
> The system helps you prepare faster and explore your library more deeply. That actually gives you more confidence to experiment and take risks.
>
> If anything, better organization of your own knowledge means you can be more creative and intuitive when you're actually playing."

---

### Q: "Why not use machine learning to suggest tags?" (Carlos, JB might ask)

**Your answer:**
> "We deliberately didn't go the AI route. This is about *you* organizing *your* library, not a system making decisions for you.
>
> Manual tagging proves that DJs think in this way already. If we added ML, we'd obscure that core insight.
>
> Post-hackathon, optional tag suggestions based on metadata could be helpful for *learning*, not decision-making."

---

### Q: "Wouldn't a simple grid of filters do this?" (JB might ask)

**Your answer:**
> "A grid of filters requires you to know what you're looking for and then click through options.
>
> Spatial browsing lets you see *everything at once* and notice things. It supports exploratory prep, not just targeted search.
>
> You're not asking 'do I have a Bridge track?' You're asking 'what do I have *in this moment*?' and discovering material you'd forgotten."

---

## If Something Goes Wrong During the Demo

**The prototype crashes or something doesn't load:**

> "This is a prototype, so sometimes things break. Let me show you the core interaction on the next view."

**[Switch to the next view or go back and try again. Stay calm and conversational.]**

> "The core idea is the three-layer navigation: timeline → spatial mood map → track details. This glitch doesn't change that insight. The question judges are really evaluating is: does organizing around *performance contexts* actually help DJs? I believe it does."

**Have a backup plan:**
- Screenshots on your phone
- A printed 1-page visual guide
- A second laptop with the demo ready

---

## Delivery Tips

### Pacing
- Slow down when introducing concepts (judges need time to absorb)
- Speed up during the walkthrough (judges want to see the flow)
- Pause after key moments (rediscovery, the arc) and let them sit
- Don't rush to fill silence; let your words land

### Eye Contact & Body Language
- Make eye contact; don't just look at the screen
- Stand to the side so judges see the demo, not you
- Use natural gestures (point to zones, gesture at spatial relationships)
- Step back when showing the full view; step closer for details
- Let your hands rest between gestures (don't fidget)

### Engagement
- Invite reactions ("Notice how spatial proximity shows relationship?")
- Pause for 3–5 seconds after big moments
- Read the room. If judges look confused, slow down or clarify
- If judges look engaged, keep momentum
- Be authentic; let your care for the problem come through

### Tone
- Conversational, not scripted
- Passionate but grounded (you're a DJ solving a real problem)
- Genuine enthusiasm for the rediscovery moment
- Humor is fine if it's natural, not forced
- Respectful of judges' expertise; invite their perspective

---

## Practice Plan

**Week of hackathon:**

**Day 1:** Write your own versions of these scripts. Make them sound like you.

**Day 2:** Practice 30-second pitch 5 times until it's natural.

**Day 3:** Practice 60-second pitch 5 times.

**Day 4:** Full 5–7 minute walkthrough with the actual prototype. Time yourself.

**Day 5:** Run the full demo in front of a friend/teammate. Get feedback.

**Before presenting:** Deliver the pitches and walkthrough once more. You should feel ready.

---

## The Secret: Authenticity

The strongest demos come from genuine care for the problem and respect for DJs as creative professionals.

You're not selling a product. You're showing how you'd solve a real problem *for yourself and your community*.

Judges—especially Richie and Carlos—can feel that authenticity. It makes your pitch memorable.

---

## Judge Resonance Summary

| Judge | What Lands |
|-------|-----------|
| **Richie Hawtin** | Respect for DJ thinking, emphasis on artistry/agency |
| **Carlos Caires** | Musical sophistication of the tagging system |
| **Yan Kalnberzin** | Spatial design clarity, intentional interaction |
| **Romain Pouillon** | Real problem, practical workflow improvement |
| **Michele Darling** | Intuitive navigation, rediscovery learning moment |
| **JB Thiebaut** | Clear scope, good product thinking, credible demo |

---

## Status

| State | Items |
|-------|-------|
| **Decided** | 30-sec, 60-sec, 5–7 min versions; artistry emphasis throughout |
| **Judge-Aligned** | Each pitch emphasizes different judge values while remaining coherent |
| **Practice Ready** | Scripts are written to be natural, not scripted |
| **Backup Plan** | What to do if tech breaks |
